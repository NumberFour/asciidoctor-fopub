...
<xsl:stylesheet version="1.0"
  ...
  <xsl:param name="body.margin.outer">10mm</xsl:param> <!-- side margin of content (right, away from binding) -->
 ...
 <!--
  ...
  <xsl:attribute-set name="footnote.mark.properties">
    ...
    <xsl:attribute name="font-size">
      <xsl:value-of select="$body.font.master * 0.6"/><xsl:text>pt</xsl:text>
    </xsl:attribute>
    <xsl:attribute name="color"><xsl:value-of select="$link.color"/></xsl:attribute>
    <xsl:attribute name="font-weight">bold</xsl:attribute>
    <xsl:attribute name="padding">0 0pt</xsl:attribute>
  </xsl:attribute-set>
